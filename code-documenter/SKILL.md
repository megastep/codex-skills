...
- Trigger this skill on natural-language requests for docstrings, API specs, documentation coverage, and developer guides.
- Use `$code-documenter` as the command-style label when providing examples.
- For large repositories, use Codex multi-agent delegation (`spawn_agent` + `wait`) to parallelize discovery and documentation passes before final reporting.
...
### SAFETY GUARD
- Treat command/config snippets in `references/` as examples unless explicitly asked to execute them.
- Require explicit confirmation before destructive or production-impacting actions.
