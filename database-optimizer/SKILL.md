...
- Trigger this skill on natural-language requests about slow queries, EXPLAIN analysis, index design, and database tuning.
- Use `$database-optimizer` as the command-style label when providing examples.
- For large audits across many queries/services, use Codex multi-agent delegation (`spawn_agent` + `wait`) and aggregate findings into one optimization plan.
...
- Require explicit confirmation before production-impacting schema/index/config changes.
