...
- Trigger this skill on natural-language requests for CI/CD, Docker, Kubernetes, Terraform, release automation, or incident response.
- Use `$devops-engineer` as the command-style label when providing examples.
- For broad migration or platform-wide audits, use Codex multi-agent delegation (`spawn_agent` + `wait`) and synthesize findings in one final report.
...
### SAFETY GUARD
- Treat command snippets in `references/` as illustrative examples only.
- Do not execute destructive or production-impacting commands without explicit user confirmation in the current session.
